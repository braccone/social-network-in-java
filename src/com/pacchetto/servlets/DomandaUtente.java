...
public class DomandaUtente {
	...
	//Query che ricerca le domande che hanno almeno un interesse in comune a quelli seguiti dall'utente e le ordina in base alla data in maniera decrescente
	 ...
	 //Query che l'id di una domanda datogli il titolo
	 ...
	 //Query che ritorna tutte le domande che hanno un interesse seguito dall'utente o che sono state fatte da un amico
	 //da rifare
	 //static String get_domandainteresseamico="SELECT dm.* FROM domanda AS dm,interesse_domanda AS ixd,interesse AS itr,interesse_utenti AS ixp, utenti AS ut, amico as am WHERE (ixp.id_persona=ut.id AND ixp.id_interesse=itr.id_interesse AND itr.id_interesse = ixd.id_interesse AND ixd.id_domanda=dm.id_domanda AND ut.id=?) OR () ORDER BY dm.data DESC";
	 ...
	 ///SELECT distinct dm.* FROM utenti as ut,amico as am,domanda as dm WHERE (dm.id_domandante=ut.id AND ut.id=?) OR (((dm.id_domandante=am.id_richiedente OR dm.id_domandante = am.id_ricevente) AND (ut.id=am.id_richiedente OR ut.id=am.id_ricevente)) AND am.accettato = 1 AND ut.id=?);
	...
	public static ResultSet getDomandante(int id_domanda) throws SQLException{
	...
	public static ResultSet getDomandeAmicoInteressi(int id_utente) throws SQLException{
		Connection conn=null;
		try {
			conn= Connessione.getConnection();
